...
namespace StarterAssets
{
    ...
    public class ThirdPersonController : MonoBehaviour
    {
        ...
        [SerializeField]
        private GameObject m_CanDriveUi;
    ...
    private PlayerInput _playerInput;
        ...
        private void Start()
        {
            ...
            m_CanDriveUi.SetActive(false);
        }

        private void StaminaUpdate()
        {
            if (Stamina < 0.0f) { Stamina = 0.0f; IsRecharging = true; }
            else if (Stamina > MaxStamina) Stamina = MaxStamina;
            StaminaBar.anchorMax = new Vector2(Stamina * percentUnit / 100f, StaminaBar.anchorMax.y);

            if (IsRecharging == false && _running == false)
            {
                TimeLastUsedStamina += Time.deltaTime;
            }

            if (TimeLastUsedStamina > TimeToRecharge && IsRecharging == false)
            {
                IsRecharging = true;
            }

            if (IsRecharging && _running == false)
            {
                Stamina += ChargeRate * Time.deltaTime;

                if (Stamina >= MaxStamina)
                {
                    TimeLastUsedStamina = 0;
                    IsRecharging = false;
                }
            }
        ...
        private void JumpAndGravity()
        {
            if (Grounded)
            {
                ...
                // Jump
                if (_input.jump && _jumpTimeoutDelta <= 0.0f && Stamina >= JumpCost && JumpOnce == false)
                {
        ...
        public void Reset()
        {
            ...
            //Vector3 distance = startPos - _controller.transform.position;
            //_controller.Move(distance);
        }
